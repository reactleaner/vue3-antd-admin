<template>
  <Col v-if="isIfShow" v-bind="schemaItem.colProps">
    <Form.Item
...
<script setup lang="tsx">
  ...
  import { computed, unref, toRefs, onMounted, getCurrentInstance } from 'vue';
  ...
  const getComponentProps = computed(() => {
    const { formModel, schemaItem } = props;
    const { componentProps = {}, component } = schemaItem;
    if (isFunction(componentProps)) {
      const compProps = componentProps({ formModel, schemaItem }) as AllComponentProps;
      compProps.placeholder ??= createPlaceholderMessage(component);
      return compProps;
    }
    if (component !== 'RangePicker') {
      (componentProps as AllComponentProps).placeholder ??= createPlaceholderMessage(component);
    }
